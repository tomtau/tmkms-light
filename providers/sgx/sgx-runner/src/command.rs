...
pub fn init2(
    ...
    let cloud_backup = match (wrap_backup_key_path, external_cloud_key_path) {
        (Some(p1), Some(p2)) => {
            let sealed_rsa_key: SealedKeyData =
                serde_json::from_slice(&fs::read(p1).expect("read wrap backup"))
                    .expect("read wrap backup");
            let backup_key: CloudBackupSeal =
                serde_json::from_slice(&fs::read(p2).expect("read wrap backup"))
                    .expect("read wrap backup");
    ...
    let request = SgxInitRequest::KeyGen2 { cloud_backup };
    let request_bytes = serde_json::to_vec(&request)
        .map_err(|e| format!("failed to convert request to json: {:?}", e))?;
...
pub fn recover2(
    ...
    if !cp.exists() {
        ...
        let cloud_backup = {
            let sealed_rsa_key: SealedKeyData =
                serde_json::from_slice(&fs::read(wrap_backup_key_path).expect("read wrap backup"))
                    .expect("read wrap backup");
            let backup_key: CloudBackupSeal = serde_json::from_slice(
                &fs::read(external_cloud_key_path).expect("read wrap backup"),
            )
            .expect("read wrap backup");
            let cloud_backup = CloudBackupKey {
                sealed_rsa_key,
        ...
        .map_err(|e| format!("failed to parse backup data: {:?}", e))?;
        let request = SgxInitRequest::CloudRecover2 {
            cloud_backup,
            key_data,
        };
        let request_bytes = serde_json::to_vec(&request)
